const SunflowerPanel = {
    ...
    checklist.style.padding = "16px 20px 0";
    ...
    const tasks = SunflowerState.getSortedTasks();

    tasks.forEach(task => {
      const daysLeft = SunflowerState.getDaysLeft(task.deadline);
      const ddayText = SunflowerState.getDdayText(daysLeft);
      const textColor = SunflowerState.getUrgencyColor(daysLeft);
      ...
      Object.assign(card.style, {
        ...
        border: isUrgent ? "1px solid #ffb3b3" : "1px solid #e5e5e5",
        borderRadius: "10px",
        padding: "10px 12px",
      ...
      card.innerHTML = `
        ...
        ">
          ...
          <div style="
            flex-shrink:0;
            background:${isUrgent ? "#cc0000" : "#eeeeee"};
            color:${isUrgent ? "#ffffff" : "#555555"};
            font-size:11px;
            font-weight:700;
            padding:4px 8px;
            border-radius:999px;
    ...
    checklist.appendChild(listWrap);
    this._panel.appendChild(checklist);
